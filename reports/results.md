...
- verdict (pre-registered rule): **falsified** — gap is not monotonically decreasing in N
...
- verdict (pre-registered rule): **supported** — M3 loses more in 6/6 clear comparisons

- verdict (adversarial audit): **inconclusive** — `audit_phase2()` divides by the mean of the unbroken baseline and discards that baseline's own two-seed spread. Propagating it drops the comparisons clearing §4's noise rule from 6 of 12 to 4 of 12, all from the `axis` break; `acceptance` clears none. See PROTOCOL amendment A3.
